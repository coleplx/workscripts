...
LAST_LIVE_CONTAINER_NAME=$(cat lxd.log | grep 'Started container' | grep -v '\-staging\-' | tail -n1 | grep -o instance=.* | cut -d'=' -f2 | cut -d' ' -f1)
LAST_LIVE_CONTAINER_TIME=$(cat lxd.log | grep 'Started container' | grep "$LAST_LIVE_CONTAINER_NAME" | tail -n1 | cut -d' ' -f1 | egrep -o '[0-9]+:[0-9]+')
